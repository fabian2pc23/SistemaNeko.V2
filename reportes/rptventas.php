...
if (!isset($_SESSION["nombre"])) {
  ...
  if ($_SESSION['ventas'] == 1) {
    ...
    {
      private $colorPrimario = array(46, 204, 113);   // Verde para ventas
      private $colorSecundario = array(39, 174, 96);
      ...
      function PanelEstadisticas($total, $totalMonto)
      {
        ...
        $this->Cell($cardWidth - 4, 6, 'TOTAL VENTAS', 0, 0, 'C');
        $this->SetFont('Arial', 'B', 24);
        ...
        $this->Cell($cardWidth - 4, 6, 'MONTO TOTAL', 0, 0, 'C');
    ...
    while ($reg = $rspta->fetch_object()) {
      $total++;
      $totalMonto += floatval($reg->total_venta);
      $datos[] = $reg;
    }
    ...
    //Título de la tabla
    $pdf->SeccionTitulo('LISTADO DE VENTAS REALIZADAS');

    $headers = array(utf8_decode('N°'), 'Fecha', 'Usuario', 'Cliente', 'Comprobante', utf8_decode('Número'), 'Total');
    ...
    foreach ($datos as $reg) {
      ...
      $pdf->Row(array(
        ...
        utf8_decode($reg->cliente),
        $reg->tipo_comprobante,
        $reg->num_comprobante,
        'S/ ' . number_format(floatval($reg->total_venta), 2)
      ), $fill);
    ...
    $pdf->Output('I', 'Reporte_Premium_Ventas_' . date('Ymd_His') . '.pdf');
?>
<?php
  } else {
