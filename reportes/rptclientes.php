  ...
  if ($_SESSION['ventas'] == 1) {
    ...
    class PDF_Premium extends PDF_MC_Table
    {
      ...
      function PanelEstadisticas($total, $activos, $inactivos)
      {
        ...
        $this->Cell($cardWidth - 4, 6, 'TOTAL CLIENTES', 0, 0, 'C');
    ...
    while ($reg = $rspta->fetch_object()) {
      $total++;
      // Asumimos que todos los clientes listados están activos
      $activos++;
      $datos[] = $reg;
    }
    ...
    //Título de la tabla
    $pdf->SeccionTitulo('LISTADO DE CLIENTES');

    $headers = array(utf8_decode('N°'), 'Nombre', 'Documento', utf8_decode('Número'), utf8_decode('Teléfono'), 'Email');
    ...
    $pdf->MultiCell(0, 6, utf8_decode(
      "El presente reporte muestra un total de $total clientes registrados en el sistema. " .
        "Este directorio permite mantener un control actualizado de la cartera de clientes " .
        "y facilita la gestión de relaciones comerciales.\n\n" .
        "Se recomienda mantener actualizada la información de contacto de cada cliente " .
    ...
    $pdf->MultiCell(0, 5, utf8_decode(
      "- Verificar periódicamente la vigencia de los datos de contacto.\n" .
        "- Implementar un programa de fidelización de clientes frecuentes.\n" .
        "- Segmentar la cartera de clientes por volumen de compras.\n" .
        "- Realizar seguimiento a clientes inactivos para reactivación."
    ), 0, 'L');

    //Mostramos el documento
    $pdf->Output('I', 'Reporte_Premium_Clientes_' . date('Ymd_His') . '.pdf');
?>
<?php
  } else {
