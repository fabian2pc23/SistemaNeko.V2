...
if (!isset($_SESSION["nombre"])) {
  ...
  if ($_SESSION['compras'] == 1) {
    ...
    {
      private $colorPrimario = array(231, 76, 60);    // Rojo para ingresos/compras
      private $colorSecundario = array(192, 57, 43);
      ...
      function PanelEstadisticas($total, $totalMonto)
      {
        ...
        $this->Cell($cardWidth - 4, 6, 'TOTAL INGRESOS', 0, 0, 'C');
        $this->SetFont('Arial', 'B', 24);
        ...
        $this->SetFont('Arial', 'B', 10);
        $this->SetXY($posX2 + 2, $y + 8);
        $this->Cell($cardWidth - 4, 6, 'MONTO TOTAL COMPRAS', 0, 0, 'C');
        $this->SetFont('Arial', 'B', 20);
        $this->SetXY($posX2 + 2, $y + 18);
        $this->Cell($cardWidth - 4, 15, 'S/ ' . number_format($totalMonto, 2), 0, 0, 'C');
    ...
    while ($reg = $rspta->fetch_object()) {
      $total++;
      $totalMonto += floatval($reg->total_compra);
      $datos[] = $reg;
    }
    ...
    $pdf->PanelEstadisticas($total, $totalMonto);

    //Título de la tabla
    $pdf->SeccionTitulo('LISTADO DE INGRESOS REGISTRADOS');

    $headers = array(utf8_decode('N°'), 'Fecha', 'Usuario', 'Proveedor', 'Comprobante', utf8_decode('Número'), 'Total');
    $widths = array(10, 22, 40, 45, 28, 22, 23);
    ...
    foreach ($datos as $reg) {
      ...
      $pdf->Row(array(
        $numero,
        $reg->fecha,
        utf8_decode($reg->usuario),
        utf8_decode($reg->proveedor),
        utf8_decode($reg->tipo_comprobante),
        $reg->num_comprobante,
        'S/ ' . number_format(floatval($reg->total_compra), 2)
      ), $fill);
    ...
    $pdf->Output('I', 'Reporte_Premium_Ingresos_' . date('Ymd_His') . '.pdf');
?>
<?php
  } else {
